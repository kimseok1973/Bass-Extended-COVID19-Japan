...
| ファイル | 内容 |
|---|---|
| `COVID_Japan_3State_Bass.ipynb` | 単一尤度版（新規陽性者のみ） |
| `COVID_Japan_3State_DualLikelihood.ipynb` | 二重尤度版（新規陽性 + 抗体保有率 + 時変報告率 ρ(t)） |
| `newly_confirmed_cases_daily.csv` | 厚労省オープンデータ（全国日次新規陽性者） |

## 二重尤度モデルの追加成果

抗体保有率（N 抗体、厚労省調査）と時変報告率 ρ(t) = sigmoid(t; t_switch) を導入した拡張版 (`*_DualLikelihood.ipynb`) では:

| 識別できた構造パラメータ | 推定値 | 解釈 |
|---|---|---|
| M | **1.05 億人** | 日本総人口（真の感受性人口） |
| ρ_late | **0.41** | 2022年後半の報告率 ~40% |
| t_switch | **2022.63** | 2022年9月末の感染者全数把握見直しと一致 |

一方、q₁（内因感染力）と p₁(t) 振幅は依然多峰。2観測では内部伝播の配分は未同定で、**変異株比率等の第3観測が必要**。
